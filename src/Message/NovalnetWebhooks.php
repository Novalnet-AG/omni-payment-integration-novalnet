...
class NovalnetWebhooks
{
    ...
    public function __construct(Gateway $gateway)
    {
        ...
        $this->paymentAccessKey  = $this->gateway->getPaymentAccessKey();

        if (empty($this->paymentAccessKey)) {
            $this->displayMessage([ 'message' => 'Payment Access key is missing!!' ]);
            return false;
        }
    ...
    public function authenticateEventData()
    {
        ...
        $this->testMode = $this->gateway->getTestMode(); // Your shop test mode value

        // Authenticating the server request based on IP.
        $requestReceivedIp = Helper::getIpAddress();
    ...
    {
        try {
            $httpresponse = new HttpResponse(json_encode($data), 200);
